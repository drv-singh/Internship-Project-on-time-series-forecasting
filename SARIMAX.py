...
panel_data = get_history(symbol='SBIN',start=start_date, end=end_date)
...
mod = sm.tsa.statespace.SARIMAX(close_df, order=(0, 1, 2), seasonal_order=(0, 2, 2,12))
results = mod.fit()
results
...
pred  = results.get_prediction(len(close_df), len(close_df)+30, dynamic=False)
pred
...
Ypred  = results.get_prediction(start = 0, end=len(close_df)-1 , dynamic=False)
...
date_pred= pd.date_range (pd.to_datetime(end_date)+timedelta(days=1), end=pd.to_datetime(end_date) + timedelta(days=31))
